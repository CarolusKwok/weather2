#' Compute path coordinates for Skew-T plot points
#'
#' The method for calculating the points via `weather2::geom_skewt_path`
#'
...
#'
#' @examples
stat_skewt_path = function(mapping = NULL, data = NULL, geom = "path",
                           position = "identity", na.rm = FALSE, show.legend = NA,
                           inherit.aes = TRUE, ...) {
  ggplot2::layer(stat = weather2::StatSkewtPath, data = data, mapping = mapping, geom = geom,
...
#'
#' Plots paths based on the `weather2::calc_skewt` function, on a Skew-T plot
#'
#' @param mapping Set of aesthetic mappings created by aes(). If specified and inherit.aes = TRUE (the default), it is combined with the default mapping at the top level of the plot. You must supply mapping if there is no plot mapping.
#' @param data The data to be displayed in this layer. There are three options:
#' * If NULL, the default, the data is inherited from the plot data as specified in the call to ggplot().
...
#'
#' @examples
geom_skewt_path = function(mapping = NULL, data = NULL,
                           position = "identity", na.rm = FALSE,
                           show.legend = NA, inherit.aes = TRUE, ...){
  ggplot2::layer(stat = weather2::StatSkewtPath, geom = ggplot2::GeomPath,
